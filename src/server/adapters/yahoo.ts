...
export async function getQuotes(symbols: string[]): Promise<Yahoo.Quote[]> {
	...
	if (error) {
		if (error.code == 'internal-error' && error.description == null) {
			...
			return _.flatten(
				await pAll(halfs.map((half) => () => getQuotes(['internal-error'].concat(half)))),
			)
		}
...
export async function syncQuotes(symbols: string[]) {
	...
	await pAll(
		chunks.map((chunk) => {
			return () =>
				getQuotes(chunk).then(function (yhquotes) {
					return redis.main.coms(
						yhquotes.map((yhquote) => {
...
export async function getSummary(symbol: string) {
	let response = (await http.get(
		`https://query1.finance.yahoo.com/v10/finance/quoteSummary/${yahoo.toSymbol(symbol)}`,
		{
