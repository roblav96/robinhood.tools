...
export function authorize(
	...
	if (missing.length > 0) {
		return Promise.reject(boom.preconditionRequired('Missing security headers' + (process.env.DEVELOPMENT ? `: '${missing}'` : '')))
	}
	...
	if (!host || host.indexOf(process.env.DOMAIN) != 0) {
		...
		return Promise.reject(boom.preconditionFailed('Invalid security header' + (process.env.DEVELOPMENT ? `: '${which}'` : '')))
	}

	let split = doc.uuid.split('.')
	if (split.length != 2) {
		return Promise.reject(boom.preconditionFailed('Invalid security header' + (process.env.DEVELOPMENT ? `: 'x-uuid'` : '')))
	}
	doc.uuid = split[0]
	if (Math.abs(Date.now() - Number.parseInt(split[1])) > 60000) {
		return Promise.reject(boom.preconditionFailed('Expired security header' + (process.env.DEVELOPMENT ? `: 'x-uuid'` : '')))
	}

	if (!doc.token) return Promise.resolve(authed);

	return redis.main.hget('security:doc:' + doc.uuid, 'prime').then(function(prime) {
