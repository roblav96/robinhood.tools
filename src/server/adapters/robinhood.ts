...
export const sync = {
	accounts({ rhtoken }: Security.Doc) {
		return paginated({ url: 'https://api.robinhood.com/accounts/', rhtoken }) as Promise<
			Robinhood.Account[]
		>
	},
	...
	achrelationships({ rhtoken }: Security.Doc) {
		return paginated({
			...
			rhtoken,
		}) as Promise<Robinhood.AchRelationship[]>
	},

	achtransfers({ rhtoken }: Security.Doc, opts = { all: false }) {
		let query = opts.all
			? {}
			: { 'updated_at[gte]': dayjs().subtract(1, 'week').format('YYYY-MM-DD') }
		return paginated({
			url: 'https://api.robinhood.com/ach/transfers/',
			query,
			rhtoken,
		}) as Promise<Robinhood.AchTransfer[]>
	},

	applications({ rhtoken }: Security.Doc) {
		return paginated({ url: 'https://api.robinhood.com/applications/', rhtoken }) as Promise<
	...
	orders({ rhtoken }: Security.Doc, opts = { all: false }) {
		let query = opts.all
			? {}
			: { 'updated_at[gte]': dayjs().subtract(1, 'week').format('YYYY-MM-DD') }
		return paginated({ url: 'https://api.robinhood.com/orders/', query, rhtoken }) as Promise<
			Robinhood.Order[]
		>
	},

	portfolios({ rhtoken }: Security.Doc) {
		return paginated({ url: 'https://api.robinhood.com/portfolios/', rhtoken }) as Promise<
			Robinhood.Portfolio[]
		>
	},
	...
	async positions({ rhtoken }: Security.Doc, opts = { all: false }) {
		...
		let positions = (await paginated({
			url: 'https://api.robinhood.com/positions/',
			query,
			rhtoken,
		})) as Robinhood.Position[]
	...
	subscriptions({ rhtoken }: Security.Doc, opts = { all: false }) {
		...
		return paginated({
			url: 'https://api.robinhood.com/subscription/subscriptions/',
			query,
			rhtoken,
		}) as Promise<Robinhood.Subscription[]>
	},

	async user({ rhtoken }: Security.Doc) {
		let user = (await http.get('https://api.robinhood.com/user/', {
			rhtoken,
			retries: 0,
		})) as Robinhood.User
		core.fix(user)
	...
	async watchlists({ rhtoken }: Security.Doc) {
		let lists = (await paginated({
			url: 'https://api.robinhood.com/watchlists/',
			rhtoken,
		})) as Robinhood.WatchlistMeta[]
		let watchlists = _.flatten(
			await pAll(
				lists.map((list) => {
					return () =>
						paginated({ url: list.url, rhtoken }) as Promise<Robinhood.Watchlist[]>
				}),
			),
		)
