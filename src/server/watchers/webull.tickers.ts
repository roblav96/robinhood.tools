...
async function syncTickers() {
	...
	await pAll(symbols.map((v, i) => () => syncTicker(v, Math.round(i / symbols.length))), { concurrency: 1 })
...
async function syncTicker(symbol: string, progress: number) {
