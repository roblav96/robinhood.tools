...
const native = (() => {
	try {
		try {
			...
			let uws = path.join(process.cwd(), 'node_modules/uws', 'build/Release/uws.node')
			if (!fs.existsSync(uws)) {
				uws = path.join(process.cwd(), 'node_modules/uws', `uws_${process.platform}_${process.versions.modules}`)
			}
			// console.log('uws ->', uws)
			return require(uws);
		}
