...
export default function procfile(pandora: Pandora.ProcfileReconcilerAccessor) {
	...
	Process(pandora.process('symbols.service').entry('./services/symbols.service'), {
		SCALE: 1,
	})
	...
	Process(pandora.process('forex.service').entry('./services/wb.quotes.service'), {
		SYMBOLS: 'FOREX',
		SCALE: 1,
	})
	Process(pandora.process('indexes.service').entry('./services/wb.quotes.service'), {
		SYMBOLS: 'INDEXES',
		SCALE: 1,
	})
