...
export function number(
	value: number,
	{ precision, price, compact, plusminus, percent, dollar, nozeros } = {} as Partial<
		Pretty.NumberFormatOptions
	>,
) {
	let auto = !Number.isFinite(precision)
	...
	if (plusminus || percent || price) auto = false
...
declare global {
	namespace Pretty {
		interface TimeFormatOptions extends prettyms.PrettyMsOptions {
			max: number
			showms: boolean
			ago: boolean
...
export function time(stamp: number, opts = {} as Partial<Pretty.TimeFormatOptions>) {
	...
	ms = ms
		.split(' ')
		.splice(0, opts.verbose ? opts.max * 2 : opts.max)
		.join(' ')
