...
export default class Lists extends Mixins(VMixin, RHMixin) {
	...
	synclists() {
		let lists = [
			{ name: 'Recently Viewed', symbols: this.recents.slice(0, 10).map((v) => v.symbol) },
		]
		return Promise.all([
			this.rhusername &&
				robinhood.sync({ synckeys: ['watchlists'] }).then((response) => {
					lists.push({
			...
			http
				.get('https://securitiesapi.webull.com/api/securities/market/tabs/1/region/6')
				.then((response: Webull.Api.HotLists) => {
					response.marketCategoryList.slice(0, 4).forEach((v) => {
						lists.push({
							name: v.name,
							symbols: v.tickerTupleArrayList.map((v) => {
