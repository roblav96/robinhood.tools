	...
	<section class="">
		<section v-visible="!busy" class="section" v-for="list in lists" :key="list.name">
			<div class="container">
				...
				<b-table
					class="card is-middle is-nowrap is-borderless is-scroll"
					hoverable
					:data="tabledata(list.symbols)"
				...
				>
					<template slot-scope="props">
						<b-table-column label="" width="1">
							<v-symbol-logo
								class="shadow is-32x32"
								:symbol="props.row.symbol"
								:acronym="props.row.acronym"
							></v-symbol-logo>
						</b-table-column>
						<b-table-column label="Symbol" width="1">
							<button
								class="button is-size-5 font-semibold leading-none w-full h-initial"
								v-on:click="gotosymbol(props.row.symbol)"
							>
								{{ props.row.symbol }}
							</button>
						</b-table-column>
						...
						<b-table-column
							...
							class="font-mono"
							numeric
							v-bull-bear="props.row.change"
						>
							{{ vnumber(props.row.change, { plusminus: true }) }} ({{
								vnumber(props.row.percent, { plusminus: true, percent: true })
							}})
						</b-table-column>
