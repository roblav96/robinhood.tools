...
export default class VSymbol extends Mixins(VMixin) {
	...
	get routes() {
		return this.$router.options.routes
			...
			.children.filter((v) => v.meta && v.meta.icon)
	}
	...
	@Vts.Watch('all.quote.price', { immediate: true }) w_price(price: number) {
		document.title =
			this.symbol +
			(price
				? ` ${this.vnumber(price)} (${this.vnumber(this.all.quote.percent, {
						plusminus: true,
						percent: true,
				  })})`
				: '')
	...
	@Vts.Watch('symbol', { immediate: true }) w_symbol(to: string, from: string) {
		this.busy = true
		return http
			.post('/quotes/alls', { symbols: [this.symbol] })
			.then((response: Quotes.All[]) => {
				console.log(
					this.symbol,
					'/quotes/alls response ->',
	...
	onstep(direction: number) {
		...
		el.scrollTo({
			left: core.math.clamp(
				el.scrollLeft + direction * (el.offsetWidth / 2),
				0,
				el.scrollWidth,
			),
			behavior: 'smooth',
	...
	get marketcap() {
		return utils.marketCap(this.all.quote.marketCap)
	}
}
