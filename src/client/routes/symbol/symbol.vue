			...
			<div class="columns items-center">
							...
							<div class="flex">
								...
								<div class="self-center">
									...
									<p v-if="all.quote.name">
										<b-tooltip
											:active="vname(all.quote.name).length > 24"
											:label="vname(all.quote.name)"
											position="is-right"
											size="is-small"
										>
					...
					<div class="columns is-mobile items-center whitespace-no-wrap text-center">
						<div class="column">
							<p>
								<v-price-ticker
									class="title"
									:price="all.quote.price"
								></v-price-ticker>
							</p>
							<p>
								<v-timestamp
									:value="all.quote.timestamp"
									:opts="{ verbose: true }"
								></v-timestamp>
							</p>
						</div>
				...
				<div class="column overflow-x-auto scrollbar">
					<div class="columns items-center is-mobile whitespace-no-wrap text-center">
						...
						<div
							v-if="Number.isFinite(all.quote.avgVolume10Day)"
							class="column is-narrow"
						>
							<p class="is-size-4">
								{{
									vnumber(all.quote.avgVolume10Day, {
										compact: true,
										precision: 1,
									})
								}}
						...
						<div
							v-if="Number.isFinite(all.quote.avgVolume3Month)"
							class="column is-narrow"
						>
							<p class="is-size-4">
								{{
									vnumber(all.quote.avgVolume3Month, {
										compact: true,
										precision: 1,
									})
								}}
						...
						<div
							v-if="Number.isFinite(all.quote.sharesOutstanding)"
							class="column is-narrow"
						>
							<p class="is-size-4">
								{{
									vnumber(all.quote.sharesOutstanding, {
										compact: true,
										precision: 1,
									})
								}}
