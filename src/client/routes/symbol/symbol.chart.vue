						...
						<b-field slot="trigger">
							<p class="control">
								<button class="button" @click="settings.time = !settings.time">
									<b-icon
										:icon="settings.time ? 'av-timer' : 'reorder-vertical'"
									></b-icon>
								</button>
							</p>
							<p class="control">
								<button class="button" @click="settings.ohlc = !settings.ohlc">
									<b-icon
										:icon="settings.ohlc ? 'poll' : 'chart-line-variant'"
									></b-icon>
								</button>
							</p>
						</b-field>
					...
					<b-field>
						<p class="control">
							<button
								class="button"
								:disabled="busy"
								@click="() => $refs.symbol_vechart.resetzoom()"
							>
						...
						<p class="control">
							<b-tooltip
								:active.sync="brushing"
								multilined
								label="Click and drag chart to crop zoom"
								size="is-small"
								animated
							>
								<button
									class="button"
									:disabled="busy"
									:class="{ 'is-primary': brushing }"
									@click="brushing = !brushing"
								>
						...
						<p class="control">
							<button
								class="button"
								:disabled="busy"
								@click="() => $refs.symbol_vechart.latestzoom()"
							>
