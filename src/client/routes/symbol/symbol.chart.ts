...
export class VSymbolEChart extends Mixins(VEChartsMixin) {
	...
	lquotes: Quotes.Live[]
	get busy() {
		return this.$parent.busy
	}
	get quote() {
		return this.$parent.quote
	}
	get settings() {
		return this.$parent.settings
	}
	get brushing() {
		return this.$parent.brushing
	}
	set brushing(brushing: boolean) {
		if (core.boolean.is(brushing)) this.$parent.brushing = brushing
	}
	...
	ctprice() {
		...
		let lquote = this.settings.time
			? this.lquotes.find((v) => v.timestamp >= ctbounds.startValue)
			: this.lquotes[ctbounds.startValue]
	...
	ctlatest(large = false) {
		...
		let threshold =
			large == true
				? ecbones.SETTINGS.largeThreshold() - 4
				: ecbones.SETTINGS.latestThreshold()
		if (this.settings.time) {
			let i = core.math.clamp(this.lquotes.length - threshold, 0, this.lquotes.length)
			bounds.startValue = this.lquotes[i].timestamp
		} else {
			bounds.start = core.math.clamp(
				core.calc.slider(this.lquotes.length - threshold, 0, this.lquotes.length),
				0,
				100,
			)
		}
	...
	build() {
		...
		let option = ecbones.option({
			...
			tooltip: [
				{ formatter: (params) => charts.tipFormatter(params as any, this.getOption()) },
			],
		})
		...
		option.series.push(
			ecbones.bar(
				{ color: theme.success, opacity: 0.5, overlap: true },
				{
					name: 'Size Bull',
					barWidth: '25%',
					xAxisIndex: 1,
		...
		option.series.push(
			ecbones.bar(
				{ color: theme.danger, opacity: 0.5, overlap: true },
				{
					name: 'Size Bear',
					barWidth: '25%',
					xAxisIndex: 1,
		...
		_.defer(() =>
			console.log(`echart build ->`, this.lquotes.length, Date.now() - stamp + 'ms'),
		)
	}
	...
	@Vts.Watch('settings.time') w_time() {
		this.reload()
	}
	...
	yPointerFormatter(params: echarts.AxisPointerParams) {
		return (
			pretty.number(params.value) +
			'\n' +
			pretty.number(core.calc.percent(params.value, this.ctprice()), {
				percent: true,
				plusminus: true,
			})
		)
...
export default class VSymbolChart extends Mixins(VMixin) {
	...
	brushing = false
	get vechart() {
		return this.$refs['symbol_vechart'] as VSymbolEChart
	}
	...
	busy = true
	@Vts.Watch('quote.tickerId') w_tickerId() {
		this.getQuotes()
	}
		...
		return Promise.resolve()
			.then(() => {
				if (this.settings.range == 'live') {
					return http
						.post('/quotes/lives', { symbols: [this.quote.symbol] })
			...
			.then((lquotes: Quotes.Live[]) => {
				...
				if (lquotes.length == 0) {
					alerts.toast(
						`Data for range '${core.string.capitalize(
							this.settings.range,
						)}' not found!`,
					)
	...
	sync1day() {
		charts
			.getChart(this.quote, this.settings.range)
			.then((lquotes: Quotes.Live[]) => {
				this.$safety()
	...
	@Vts.Watch('settings', { deep: true }) w_settings(
		settings: typeof VSymbolChart.prototype.settings,
	) {
	...
	get rangeindex() {
		return this.ranges.indexOf(this.settings.range)
	}
	vrange(range: string) {
		return charts.range(range)
	}
