...
<template>
	<nav class="navbar is-fixed-top is-unselectable border border-b-1 desktop:px-4" role="navigation" aria-label="main navigation">
				...
				<div class="navbar-start mr-0">
					<b-tooltip :active="breakpoints.desktopAndUp&&route.name!=$route.name" :label="vcapitalize(route.name)"
						...
						<router-link class="navbar-item flex items-center h-full" :to="{name:route.name}">
							...
							<span :class="{'is-hidden-desktop':route.name!=$route.name,'font-medium':route.name==$route.name}" class="ml-2">
