...
export const string = {
	...
	filterwords(value: string, words: string[]) {
		return (
			...
			value
				.split(' ')
				.filter((v) => !words.includes(string.alphanumeric(v)))
				.join(' ')
				.trim()
		)
	},
	minify(value: string) {
		...
		return value
			.split(' ')
			.filter((v) => STOP_WORDS.indexOf(v) == -1)
			.join(' ')
			.trim()
	},
	tags(value: string, input = [] as string[]) {
		...
		return _.uniq(_.compact(tags)).filter(
			(v) => v && v.length > 2 && STOP_WORDS.indexOf(v) == -1,
		)
	},
	id(value: string) {
