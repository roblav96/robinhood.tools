...
Object.keys(HttpConfig).forEach((k) => {
	if (!HttpConfig[k]) delete HttpConfig[k]
})
...
export function applyconfig(config: Partial<Http.Config>) {
	...
	if (config.verbose || config.debug) {
		let ending =
			config.query || config.body
				? ' -> ' + JSON.stringify(config.query || config.body || '').substring(0, 64)
				: ''
		...
		.catch(function (error: boom) {
			...
			if (error.data) {
				message += `\nerror.data -> ${JSON.stringify(
					_.get(error, 'data.data', error.data),
				)}`
			}
...
export function post<B = any, T = any>(
	url: string,
	body?: B,
	config = {} as Partial<Http.Config>,
): Promise<T> {
	config.url = url
	config.method = 'POST'
	...
	return new Promise(function (resolve, reject) {
		simple.concat(config, function (error, res, data: any) {
			...
			if (error || res.statusCode >= 400) {
				...
				return reject(
					new boom(error || res.statusMessage, {
			...
			if (config.proxify) {
				...
				data = _.get(data, 'json.json') || data
			}
...
declare module 'simple-get' {
	interface RequestOptions extends Partial<typeof HttpConfig> {}
}
declare global {
	namespace NodeJS {
		interface Global {
