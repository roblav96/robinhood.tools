...
export function applyFull(
	...
	core.object.merge(quote, {
		...
		exchange: core.fallback(
			iexitem.exchange,
			iexitem.primaryExchange,
			wbticker.exchangeName,
			yhquote.fullExchangeName,
			wbticker.disExchangeCode,
			wbticker.exchangeCode,
		),
		currency: core.fallback(wbticker.currencyCode, wbquote.currency),
		country: core.fallback(
			instrument.country,
			wbticker.regionIsoCode,
			wbquote.countryISOCode,
			wbquote.regionAlias,
		),
		sharesOutstanding: _.round(
			core.fallback(
				wbquote.totalShares,
				yhquote.sharesOutstanding,
				iexitem.sharesOutstanding,
			),
		),
	...
	quote.avgVolume10Day = _.round(
		core.fallback(wbquote.avgVol10D, yhquote.averageDailyVolume10Day),
	)
	quote.avgVolume3Month = _.round(
		core.fallback(wbquote.avgVol3M, yhquote.averageDailyVolume3Month),
	)
	quote.avgVolume = _.round(
		core.fallback(
			wbquote.avgVolume,
			iexitem.avgTotalVolume,
			quote.avgVolume3Month,
			quote.avgVolume10Day,
			wbquote.volume,
			yhquote.regularMarketVolume,
		),
	)
...
export function applyWbQuote(
	quote: Quotes.Calc,
	wbquote: Webull.Quote,
	toquote = {} as Quotes.Calc,
) {
...
declare global {
	namespace Quotes {
		type AllKeys = keyof typeof ALL_RKEYS
	}
}
...
declare global {
	namespace Quotes {
		type ITiny = typeof TINY
		interface Tiny extends ITiny {}
	}
}
const TINY = {
	symbol: string,
...
declare global {
	namespace Quotes {
		...
		interface Live extends ILive, Tiny {}
	}
}
const LIVE = {
	stamp: number,
